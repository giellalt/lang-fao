...
if test -z "$GIELLA_CORE" ; then
    ...
    exit 1
elif ! test -x src/fst/morphology/test/generate-noun-lemmas.sh ; then
    echo "test src/fst/morphology/test/generate-noun-lemmas.sh not found"
...
srcdir=. src/fst/morphology/test/generate-noun-lemmas.sh > "$tmplog"
